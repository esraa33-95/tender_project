...
class StoreMaterialCategory extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            function ($attribute, $value, $error) {
                if (MaterialCategoryTranslation::where('name', $value)->where('locale', 'en')->exists()) 
                {
        ...
        'name_ar' => ['required','string','min:2','max:255', 'regex:/^[\p{Arabic} ]+$/u',
            function ($attribute, $value, $error) {
                if (MaterialCategoryTranslation::where('name', $value)->where('locale', 'ar')->exists())
                 {
                    $error(__('validation.custom.name_ar.unique'));
                }
            }
        ],
        'room_property'=>['required','in:1,2,3'],
